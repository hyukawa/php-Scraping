    ...
    if ($pos !== false) {
        ...
        if($pos !== false) {
            ...
            if($pos !== false) {
                // 遅延であればLED色は黄色で点滅
                //$led_pat += [ "rail_state" => "▲" ];
                $led_pat += [$line_name[$j]][ "rail_state" => $railway_info[$i+1] ];
                $led_pat += [$line_name[$j]][ "rail_state_detail" => $railway_info[$i+2] ];
                 break;
            } else {
...
echo "運行状況： " . $led_pat["江ノ島電鉄線"]["rail_state"] . "</br>";
echo "遅延情報： " . $led_pat["江ノ島電鉄線"]["rail_state_detail"] . "</br>";
